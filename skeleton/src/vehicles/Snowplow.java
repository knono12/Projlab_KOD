...
/**
 * A hókotrót reprezentáló osztály, amely a {@link Vehicle} ősosztályból származik.
 * A hókotró feladata a város útjainak (sávjainak) tisztítása. Működése a rászerelt fej ({@link Attachment}) típusától és a sáv állapotától függ.
...
public class Snowplow extends Vehicle implements Purchasable{
     ...
     * @param name A hókotró azonosító neve a szkeletonhoz.
     */
    public Snowplow(int price, String name){
        super(name);
        this.price = price;
    }
     ...
     * A szabályok szerint a hókotró jelenléte megakadályozza a balesetet, ezért mindig hamis.
     * @return Mindig false.
     */
    @Override
    ...
    public void changeAttachment(Attachment newAttachment){
        SkeletonManager.call(name + ".changeAttachment(" + newAttachment.getName() + ")");
     ...
     *  A Skeletonban most csak hányófejet tud cserélni, azért mert a valós játékban majd felhasználó választhat, most csak a folyamatot mutatja be.
     */
    public void onStation() {
        SkeletonManager.call(name + ".onStation()");
     ...
     * A hókotró eltárolja a takarító referenciáját, hogy később a sikeres takarításért neki adhassa a jutalmat, a takarító pedig felveszi a hókotró listájába.
